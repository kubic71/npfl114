...
'''
Based on the implementation here : https://github.com/Lasagne/Recipes/blob/master/papers/densenet/densenet_fast.py.py
'''
...
def conv_block(ip, nb_filter, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Convolution2D(nb_filter, 3, 3, padding="same", kernel_regularizer=l2(weight_decay))(x)
    if dropout_rate:
...
def transition_block(ip, nb_filter, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Convolution2D(nb_filter, 1, 1, padding="same",
...
def createDenseNet(nb_classes, img_dim, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=16, dropout_rate=None,
    ...
    # Initial convolution
    x = Convolution2D(nb_filter, 3, 3, padding="same", name="initial_conv2D" , kernel_regularizer=l2(weight_decay))(model_input)
    ...
    x = Dense(nb_classes, activation='softmax', kernel_regularizer=l2(weight_decay), b_regularizer=l2(weight_decay))(x)
