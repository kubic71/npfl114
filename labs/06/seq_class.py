...
class Network:
    def __init__(self, args):
        inputs = tf.keras.layers.Input(shape=[args.sequence_length, args.sequence_dim])
        ...
        if args.rnn_cell == "SimpleRNN":
            cell = tf.keras.layers.SimpleRNNCell(units=args.rnn_cell_dim)
        elif args.rnn_cell == "LSTM":
            cell = tf.keras.layers.LSTMCell(units=args.rnn_cell_dim)
        elif args.rnn_cell == "GRU":
            cell = tf.keras.layers.GRUCell(units=args.rnn_cell_dim)
        else:
            raise Exception("Unknown RNN Cell type")

        last = tf.keras.layers.RNN(cell=cell, return_sequences=True, dtype=tf.float32)(inputs)
        ...
        if args.hidden_layer is not None:
            last = tf.keras.layers.Dense(args.hidden_layer, "relu")(last)

        # TODO: Generate predictions using a fully connected layer
        # with one output and `tf.nn.sigmoid` activation.
        outputs = tf.keras.layers.Dense(1, activation="sigmoid")(last)
    ...
    def train_batch(self, batch, clip_gradient):
        ...
        with tf.GradientTape() as tape:
            outputs = self.model(batch["sequences"], training=True)
            loss_fn = self._loss(batch["labels"], outputs)
            gradients = tape.gradient(loss_fn, self.model.trainable_variables)
            grad_norm = 0
            if not clip_gradient:
                grad_norm = tf.linalg.global_norm(gradients)
            else:
                gradients, grad_norm = tf.clip_by_global_norm(gradients, clip_gradient)
            ...
            with self._writer.as_default():
                for name, metric in self._metrics.items():
                    if name == "loss":
                        tf.summary.scalar("train/" + name, loss_fn)
                    else:
                        metric(batch["labels"], outputs)
                        tf.summary.scalar(
