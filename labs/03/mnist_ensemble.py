...
with open("mnist_ensemble.out", "w") as out_file:
    for model in range(args.models):
        ...
        current_pred = sum(predictions)
        pred_labels = []
        for pred in current_pred:
            pred_labels.append(list(pred).index(max(pred)))

        pred_labels = np.array(pred_labels)
        right = sum(pred_labels == mnist.dev.data["labels"])
        # TODO: Compute the accuracy on the dev set for
        ...
        ensemble_accuracy = right / mnist.dev.size
